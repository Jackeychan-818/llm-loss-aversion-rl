...
6. **Dynamic sampling is critical** — the base almost always outputs "No", so
   many G=16 groups may be all-identical and have zero differentiated
   task-reward advantage. With `beta=0.04`, a KL-only gradient may remain; do
   not call this a guaranteed zero total gradient. Treat training reward as a
   diagnostic and select checkpoints by the frozen structural rule.
