...
matched base. A one-seed 6,000-step pilot suggests that both matched SFT and
sign-only GRPO can also sharply reduce the validation estimate; this is
exploratory and weakens any GRPO-necessity claim. These are results completed
so far, not a finished thesis: confirmatory matched baselines, robust
structural inference, IFEval, trained-policy preference agreement, and
complete OOD artifact archival remain pending.
\end{abstract}
...
Matched SFT and sign-only GRPO & Optimization baselines & \partdone & One-seed 6k validation pilot complete; full two-seed 30k runs and a new untouched suite remain. \\
...
task-reward advantage and are filtered. This DAPO-style behavior
\citep{yu2025dapo} avoids a misleading task-reward direction but does not avoid
the generation cost of all-identical groups. Because the objective retains a
KL coefficient of 0.04, such a group can still contribute a KL-only gradient;
``zero task advantage'' does not imply a guaranteed zero total update.
...
Loss and scaling & DAPO; no reward scaling & Zero-variance groups carry no differentiated task advantage; KL-only update may remain. \\
...
\subsection{Exploratory causal-baseline pilot}

A predeclared low-cost pilot trained one SFT seed and one sign-only GRPO seed
through 6,000 steps, then evaluated steps 2,000, 4,000, and 6,000 on
\texttt{test\_goods}. No frozen selector, OOD-50 evaluation, or prospective
unused-configuration evaluation was run for these models.
...
\toprule
Method & Step & $\widehat\lambda$ & $\widehat\eta$ & $d$ & Cons. & $W$ \\
\midrule
SFT & 2k & $-0.160$ & $-0.466$ & 0.493 & 0.623 & 0.875 \\
SFT & 4k & 0.059 & 0.012 & 0.060 & 0.718 & 0.900 \\
SFT & 6k & 0.047 & 0.023 & 0.052 & 0.725 & 0.901 \\
Sign-only GRPO & 2k & 0.147 & 0.062 & 0.160 & 0.664 & 0.885 \\
Sign-only GRPO & 4k & 0.558 & $-0.064$ & 0.562 & 0.633 & 0.887 \\
Sign-only GRPO & 6k & 0.388 & 0.039 & 0.390 & 0.660 & 0.891 \\
\bottomrule
\end{tabular}
\caption{Exploratory seed-1 causal-baseline pilot on validation data, where
$d=\sqrt{\lambda^2+\eta^2}$. It does not select a checkpoint or establish a
method winner.}
\label{tab:causal-pilot}
\end{table}

Both methods move the validation behavior far from the matched base
($\widehat\lambda=7.637$). SFT reaches $d\approx0.05$ by 4k--6k, while the
sign-only trajectory is noisier. If the SFT pattern replicates, the defensible
claim is that targeted post-training works, not that GRPO is necessary. The
current sign-only contrast also changes the global reward scale from
$\pm|\dd|$ to $\pm1$ while reward-scale normalization is disabled, so it does
not isolate magnitude information from update scale. Full two-seed 30k runs,
a scale-matched sign control, a newly frozen comparison suite, and complete
pilot provenance remain required.
...
\subsection{Framing specificity: adverse transfer}

A separate non-gating benchmark holds the decision content fixed while varying
positive versus negative framing across 120 scenarios and 23 paired
realizations per scenario. The exploratory Qwen-derived step-8k model is
\emph{more}, not less, framing-sensitive than the exact local base.
...
\begin{tabular}{lrr}
\toprule
Metric & Exact local base & Qwen-derived GRPO step 8k \\
\midrule
Hard choice flip rate & 0.505 & 0.689 \\
Mean absolute probability gap & 0.490 & 0.679 \\
Probability monotonicity violation rate & 0.245 & 0.258 \\
Hard-choice monotonicity violation rate & 0.0055 & 0.0061 \\
\bottomrule
\end{tabular}
\caption{Completed framing-specificity benchmark. Larger values are worse for
all four metrics. This is an adverse boundary result, not a failure of the
paired endowment result.}
\label{tab:framing}
\end{table}

The result rules out a broad ``general debiasing'' interpretation. The current
intervention is specific to the ownership-conditioned decision policy it was
trained on and may worsen sensitivity to other surface frames. Derived
comparison metrics are committed under
\texttt{framing/full\_qd8k\_120x23/}; raw predictions are checksummed but not
committed.
...
\midrule
Matched Qwen-derived SFT & Required; pilot complete & Are binary supervised targets sufficient without GRPO across seeds and untouched data? \\
...
Sign-only and scale-matched GRPO & Required; pilot partial & Does pseudo-utility magnitude add value beyond target sign and global reward scale? \\
...
The evidence does not establish that GRPO is necessary. The exploratory SFT
pilot produces a comparable validation shift by 4k--6k, so a GRPO-specific
claim is currently disfavored unless the full comparison changes that result.
Training reliability is supported by two
...
ground truth. The adverse framing result also prevents interpreting the
intervention as general debiasing.
...
intervention substantially changes ownership-dependent choice across one
exploratory and two confirmatory runs, and retains low estimates on new goods
and prospectively unused configurations. GSM8K shows no material loss under
the frozen seed gates, whereas the framing benchmark exposes an adverse
boundary condition. The next phase determines whether GRPO adds value over
supervised training and whether the structural, semantic-robustness, and
capability conclusions survive stronger uncertainty estimates and reproducible
artifact release.
