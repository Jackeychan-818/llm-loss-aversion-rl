...
The project fine-tunes **Qwen2.5-7B-Instruct** with **GRPO** using a rationality-based reward. The main held-out result is large: structural estimation moved Qwen from **λ̂_before = 11.75** to **λ̂_after = 0.177**, a **98.5% reduction**.
...
Held-out evaluation on `test_goods.json` using `eval/run_qwen_local.py` and the Model A estimators.
...
| 4. Post-training evaluation | ✅ Done | Baseline/debias/forced treatment estimates |
...
- Run ablations for reward source, KL β, learning rate, LoRA rank, and out-of-distribution goods.
